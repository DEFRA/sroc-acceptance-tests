      ...
      cy.get(`.table-responsive > tbody > tr:nth-child(${index + 1}) td`).eq(2).invoke('text').then((email) => {
        if (email === user.email) {
          cy.get(`.table-responsive > tbody > tr:nth-child(${index + 1})`).invoke('attr', 'id').then((id) => {
            ...
            AddUserPage.enabledCheckbox().click()
            AddUserPage.submitButton().click()
          })
        }
      })
...
And('I can confirm the users account is disabled', (regime) => {
  cy.get('@user').then((user) => {
    cy.signOut()
    ...
    cy.get('@lastEmail').then((lastEmail) => {
      ...
      cy.visit(link).then(() => {
        ...
        cy.get('.col > .alert-warning').should('contain.text', '\n          \n            ×\n          \n          Your account is not enabled.  Contact your administrator.\n        ')
      })
    })
  })
})

When('I enable the users account', () => {
  TransactionsPage.adminMenu.getOption('User Management', '').click()
    ...
    cy.get('@lastEmail').then((lastEmail) => {
      ...
      cy.visit(link).then(() => {
        ...
        cy.get('@user').then((user) => {
          ...
          cy.alertShouldContain('Your password was set successfully. You are now signed in.')
        })
      })
    })
  })
